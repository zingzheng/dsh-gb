...
export class PhoneRemoteServer {
  ...
  constructor(hooks, options = {}) {
    ...
    // 公网覆盖地址（隧道运行时可替代局域网主机地址；null = 用局域网地址）
    this.publicBase = null;
    // 隧道状态（host 侧注入，仅随 /info 上报给设置页）
  ...
  urlFor(ip) {
    const base = this.publicBase ?? `http://${ip}:${this.port}`;
    return `${base}/?t=${this.token}`;
  }

  /** 覆盖二维码/URL 的地址基座（公网隧道运行时用；null/非字符串恢复局域网地址）。 */
  setPublicBase(url) {
    this.publicBase = typeof url === 'string' && url !== ''
      ? url.replace(/\/+$/, '')
      : null;
  }

  /** 上报隧道状态（{state, mode, url, error}，仅用于 /info 展示）。 */
  setTunnelInfo(info) {
    this.tunnelInfo = info === null || info === undefined ? null : { ...info };
  }
  ...
  async handleRequest(req, res) {
    ...
    if (path === '/info') {
      ...
      json(res, 200, {
        ...
        url: this.urlFor(this.host ?? '127.0.0.1'),
    ...
    if (path === '/state') {
      // 轮询降级接口：与 SSE 首帧完全相同的合并状态（手机页在 SSE 不可达时使用）
      const token = tokenOf(req, url.searchParams);
      if (!safeEqual(token, this.token)) {
        json(res, 403, { ok: false, error: 'forbidden' }, true);
        return;
      }
      json(res, 200, this.mergedState(), true);
      return;
    }
    if (path === '/events') {
      const token = tokenOf(req, url.searchParams);
