...
import { PhoneRemoteServer, isLoopback } from './server-core.js';
...
// 插件配置持久化（公网隧道等）：机器级，写法清晰可手改；原子写（tmp + rename）。
const CONFIG_FILE = path.join(process.env.DSH_HOME ?? path.join(os.homedir(), '.dsh'), 'dsh-gb.json');

/** 读取插件配置；缺失/损坏时返回空对象（区别于非法 JSON，避免整插件不可用）。 */
function loadConfig() {
  try {
...
function saveConfig(config, log) {
  try {
    const tmp = `${CONFIG_FILE}.tmp`;
    writeFileSync(tmp, JSON.stringify(config, null, 2) + '\n', 'utf8');
    renameSync(tmp, CONFIG_FILE);
  } catch (error) {
    log('error', `配置持久化失败: ${error instanceof Error ? error.message : String(error)}`);
  }
}

/** 小请求体读取（JSON 控制接口用，8KB 上限）。 */
...
/** 校验并规范化隧道设置（enabled/mode/token/publicHost）；非法返回 null。 */
function normalizeTunnel(raw) {
  if (raw === null || typeof raw !== 'object') return null;
  if (typeof raw.enabled !== 'boolean') return null;
  const mode = raw.mode === 'named' ? 'named' : raw.mode === 'quick' ? 'quick' : null;
  if (mode === null) return null;
  const token = typeof raw.token === 'string' ? raw.token : '';
  const publicHost = typeof raw.publicHost === 'string' ? raw.publicHost : '';
  if (token.length > 1000 || publicHost.length > 500) return null;
  if (mode === 'named') {
    if (token === '' || token.length < 20) return null; // cloudflared 命名隧道 token 是长字符串
    if (publicHost === '') return null;
    try {
      const u = new URL(publicHost);
      if (u.protocol !== 'http:' && u.protocol !== 'https:') return null;
    } catch { return null; }
  }
  return { enabled: raw.enabled, mode, token, publicHost };
}
...
export function apply(ctx) {
  ...
  // ── 公网隧道（路线 A）：cloudflared 快速/命名隧道，随插件自启自愈 ──
  const config = loadConfig();
  const tunnelConfig = {
    enabled: config.tunnel?.enabled === true,
    mode: config.tunnel?.mode === 'named' ? 'named' : 'quick',
    token: typeof config.tunnel?.token === 'string' ? config.tunnel.token : '',
    publicHost: typeof config.tunnel?.publicHost === 'string' ? config.tunnel.publicHost : '',
  };
  const persistTunnelConfig = () => { saveConfig({ ...config, tunnel: tunnelConfig }, log); };
  const tunnel = new TunnelManager({ log, cacheDir: path.join(path.dirname(TOKEN_FILE), 'cache', 'dsh-gb') });
  tunnel.onPhase((info) => {
    server.setTunnelInfo(info);
    // 隧道地址只在 running 时可信；启动/失败期间 QR 回退局域网地址
    server.setPublicBase(info.state === 'running' && info.url ? info.url : null);
  });
  const applyTunnel = () => {
    if (!tunnelConfig.enabled) { tunnel.stop(); return; }
    if (typeof server.port !== 'number' || server.port === 0) return; // 等服务器就绪
    tunnel.start({
      mode: tunnelConfig.mode,
      token: tunnelConfig.token,
      publicHost: tunnelConfig.publicHost,
      port: server.port,
    }).catch((error) => {
      log('error', `隧道启动失败: ${error instanceof Error ? error.message : String(error)}`);
    });
  };
    ...
    const registerAll = (webServer) => {
      ...
      register('/phone-remote/info', (req, res) => {
        ...
        const body = JSON.stringify({
          ...
          // URL 内含令牌：非回环来源一律脱敏（远端只读状态，不能取二维码/配置）
          url: loopback ? server.urlFor(server.host ?? '127.0.0.1') : null,
          restricted: !loopback,
          phonesOnline: server.phones.size,
          bridgeOnline: server.bridges.size > 0,
          tunnel: server.tunnelInfo,
        });
        res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
        res.end(body);
      });
      register('/phone-remote/tunnel', (req, res) => {
        // 控制端点仅限回环：局域网/远程来源不得读取或改动隧道配置
        ...
        if (req.method === 'GET') {
          ...
          res.end(JSON.stringify({
            ...
            url: server.urlFor(server.host ?? '127.0.0.1'),
          }));
          return;
        }
        if (req.method !== 'POST') {
        ...
        readJsonBody(req).then((raw) => {
          const next = normalizeTunnel(raw);
          if (next === null) {
            res.writeHead(400, { 'content-type': 'application/json; charset=utf-8' });
            res.end(JSON.stringify({ ok: false, error: 'bad-config' }));
            return;
          }
          tunnelConfig.enabled = next.enabled;
          tunnelConfig.mode = next.mode;
          tunnelConfig.token = next.token;
          tunnelConfig.publicHost = next.publicHost;
          persistTunnelConfig();
          applyTunnel();
          log('info', `隧道设置更新: enabled=${next.enabled} mode=${next.mode}`);
          res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store' });
          res.end(JSON.stringify({ ok: true, tunnel: tunnel.status(), url: server.urlFor(server.host ?? '127.0.0.1') }));
        }).catch((error) => {
          res.writeHead(400, { 'content-type': 'application/json; charset=utf-8' });
