...
/* 顶部品牌（Game Boy 主体之外） */
.brand { text-align: center; font: 800 20px/1.2 ui-monospace, "Courier New", monospace;
  letter-spacing: 6px; color: #e9e2d2; text-shadow: 0 1px 0 rgba(0,0,0,.35); }
.brand b { color: #ffd23e; text-shadow: 0 1px 0 rgba(0,0,0,.45); }

/* 顶部：工作区/会话选择条（与屏幕共用 LCD/边条，融入机身） */
.topbar { display: flex; flex-direction: column; gap: 7px; }
.selrow { display: flex; gap: 8px; align-items: stretch; }
.selbox { flex: 1; min-width: 0; }
.selbox select { width: 100%; height: 40px; -webkit-appearance: none; appearance: none; background: var(--lcd);
  border: 2px solid var(--frame); border-radius: 4px; color: var(--lcd-fg); font: 700 15px/1.4 ui-monospace, "Courier New", monospace;
  padding: 0 6px; text-overflow: ellipsis; white-space: nowrap; text-align: center; }
.selbox select:focus { outline: 2px solid #2b4d1f; }
.btnnew { flex: 0 0 auto; width: 46px; border-radius: 12px;
  background: linear-gradient(180deg, var(--btn), var(--btn2)); border: 2px solid var(--frame);
  color: var(--ink); font: 800 20px/1 ui-monospace, "Courier New", monospace; cursor: pointer; }
.btnnew:active { background: var(--btn2); transform: translateY(1px); }
...
/* 底部按键区 */
.padwrap { display: flex; gap: 10px; align-items: stretch; padding: 12px 4px 2px; }
.dpadwrap { flex: 1 1 46%; }
.dpad { display: grid; grid-template-columns: repeat(3, 1fr); grid-template-rows: repeat(3, 1fr); gap: 4px; aspect-ratio: 1; }
.dbtn { border-radius: 10px; background: linear-gradient(180deg, var(--btn), var(--btn2)); border: 2px solid var(--frame);
  color: var(--ink); font: 800 26px/1 ui-monospace, "Courier New", monospace; cursor: pointer; min-height: 0; }
...
<body>
<div class="brand">DSH <b>掌机</b></div>
<div id="gbshell">
  <div class="topbar">
    <div class="selrow">
      <div class="selbox"><select id="wsSel"></select></div>
      <div class="selbox"><select id="sessSel"></select></div>
      <button class="btnnew" id="btnNew" aria-label="新建会话">＋</button>
    </div>
  </div>
...
(function () {
  ...
  var state = { connected: false, received: false };
  var qIndex = 0;            // 当前题目序号
  ...
  function renderDrops() {
    ...
    if (itemsWs.length === 0) {
      ...
      oEmpty.value = '';
      oEmpty.textContent = '（无工作区）';
      oEmpty.selected = true;
      ws.appendChild(oEmpty);
    }
    ...
    ss.textContent = '';
    if (itemsS.length === 0) {
      var oEmptyS = document.createElement('option');
      oEmptyS.value = '';
      oEmptyS.textContent = '（新会话）';
      oEmptyS.selected = true;
      ss.appendChild(oEmptyS);
    }
  ...
  function renderLcd() {
    ...
    if (box === null) return;
    if (!state.connected) {
      // 从未收到过任何状态 → 「正在连接…」；曾收到而后断连 → 「× 未连接」
      box.innerHTML = '<div class="row dim">' + (state.received ? '× 未连接' : '正在连接……') + '</div>';
      return;
    }
  ...
  var pollTimer = null;
  var announcedPoll = false;

  function fetchState() {
    return fetch('/state?t=' + encodeURIComponent(token))
      .then(function (r) { return r.json(); })
      .then(function (next) {
        onMessage({ data: JSON.stringify(next) });
        return next;
      })
      .catch(function () { /* 网络抖动：保持现状 */ });
  }

  function startPolling() {
    if (pollTimer !== null) return;
    fetchState(); // 立即拉一次，不等轮询间隔
    pollTimer = setInterval(fetchState, 2500);
    if (!announcedPoll) {
      announcedPoll = true;
      showToast('实时推送不可用，已切换轮询模式', 'ok');
    }
  }
  ...
  function connect() {
    ...
    var gotFrame = false;
    es.onmessage = function (event) { gotFrame = true; onMessage(event); };
    es.onerror = function () { state.connected = false; render(); };
    // 部分代理/隧道（如 Cloudflare 快速隧道）只透传 SSE 响应头、缓冲或丢弃数据体：
    // 4 秒未收到任何事件帧即降级为 /state 轮询（普通 JSON 响应可正常到达）
    setTimeout(function () {
      if (gotFrame) return;
