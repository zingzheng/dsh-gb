...
function fetchInfo() {
  ...
  bridgeBasePromise = (async () => {
    ...
    if (same !== null && same.ok === true) {
      ...
      return { port: same.port, token: tokOf(same.url), url: same.url };
    }
    for (let port = 7788; port <= 7795; port++) {
      try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 1200);
        const r = await fetch(`http://127.0.0.1:${port}/info`, { cache: 'no-store', signal: controller.signal })
          .then((res) => (res.ok ? res.json() : null));
        clearTimeout(timer);
        if (r !== null && r.ok === true) {
          console.log('[dsh-gb] info(扫描)', JSON.stringify(r));
          return { port, token: tokOf(r.url), url: r.url };
        }
      } catch {
        continue;
      }
  ...
  function SettingsPage() {
    ...
    const refresh = () => {
      ...
      fetchInfo().then((base) => {
        ...
        fetch(`http://127.0.0.1:${base.port}/info`, { cache: 'no-store' })
          .then((r) => r.json())
          .then((live) => {
            setInfo({
              ...base,
...
function makeRootBridge(sessionsService, workspacesService) {
  function RootBridgeView(props) {
    ...
    const push = () => {
      ...
      fetchInfo().then((base) => {
        if (base === null) return;
        fetch(`http://127.0.0.1:${base.port}/bridge/state?t=${encodeURIComponent(base.token)}`, {
          method: 'POST',
    ...
    React.useEffect(() => {
      ...
      const open = () => {
        fetchInfo().then((base) => {
          ...
          const url = `http://127.0.0.1:${base.port}/bridge/events?t=${encodeURIComponent(base.token)}`;
  ...
  function SessionRelayView(props) {
    ...
    const pushPending = () => {
      ...
      fetchInfo().then((base) => {
        ...
        fetch(`http://127.0.0.1:${base.port}/bridge/pending?t=${encodeURIComponent(base.token)}`, {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
